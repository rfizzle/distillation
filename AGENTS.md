...
Distillation is a Minecraft 1.21.1 Fabric mod — an alchemy overhaul that makes
brewing a system you learn by playing: in-stand recipe discovery with vapor
hints and instructive failures, brewable recipes for the effect roster vanilla
shipped without them, six-bottle batch brewing over a heated cauldron, honest
...
- [HUD-STANDARD.md](https://github.com/rfizzle/concord/blob/master/HUD-STANDARD.md) — Distillation has **no HUD slot, by design** (`design/DESIGN.md` §2); it ships no HUD element and no HUD accessors
...
- **Mixin config:** `distillation.mixins.json` in `src/main/resources` for
  common/server mixins, plus a client-only `distillation.client.mixins.json` in
  `src/client/resources`. Mixin package: `com.rfizzle.distillation.mixin`.
- **One brew choke point:** every potion resolution — normal, murky, batch,
  discovery recording, and the `DistillationBrewCallback` — flows through the
  single brew-completion seam (`design/SPEC.md` §1 Implementation Notes).
  Never add a second path that produces a brewed bottle.
...
- **Commits:** [Conventional Commits](https://www.conventionalcommits.org/)
  with a topical scope naming the feature area: `feat(discovery): …`,
  `feat(brews): …`, `feat(batch): …`, `feat(draughts): …`, `feat(premium): …`,
  `feat(antidotes): …`, `feat(thrown): …`, `docs(design): …`, `chore(ai): …`.
