...
# reference to an unset variable aborts first, with a worse message than the clean
# usage error the guard exists to print.
NEW_VERSION=""
for arg in "$@"; do
  case "$arg" in
    --no-push) NO_PUSH=1 ;;
...
git -C "$PROJECT_ROOT" tag -a "$TAG" -m "Distillation $NEW_VERSION"
