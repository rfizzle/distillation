...
/**
 * Versioned raw-JSON schema migration for {@link DistillationConfig}. Migrations run on the raw
 ...
 * {@link DistillationConfig} in lockstep, append the lambda (never reorder), and add a
 * {@code ConfigMigratorTest} case (legacy→new, idempotency, already-current passthrough).
 *
 * <p>Migration runs only on the file-load path. A config built from {@code toSyncJson}/{@code
 * fromJson} (ModMenu working copy, server→client sync) is already current — never re-run
 * migration on it.
 */
    ...
    static boolean migrate(JsonObject json) {
        ...
        for (int i = version; i < CURRENT_VERSION && i < MIGRATIONS.length; i++) {
            try {
                ...
                Distillation.LOGGER.info("Migrated config from v{} to v{}", i, i + 1);
                changed = true;
            } catch (Exception e) {
                Distillation.LOGGER.warn("Config migration v{} to v{} failed; skipping: {}", i, i + 1, e.getMessage());
            }
        }
        if (changed) {
